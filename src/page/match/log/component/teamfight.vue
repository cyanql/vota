<template>
	<div class="log-teamfight">
		<div class="log-players" @click="onClick">
			<div class="team">
				<figure v-for="player in log.radiantPlayers" class="hero square" :class="player.deaths && 'death'" v-if="player.damage" :style="`background-image: url(${player.heroImage})`"></figure>
			</div>
			<div class="team">
				<figure v-for="player in log.direPlayers" class="hero square" :class="player.deaths && 'death'" v-if="player.damage" :style="`background-image: url(${player.heroImage})`"></figure>
			</div>
			<div class="log-info">
		...
		<expand-transition>
			<div class="log-detail" v-show="detailVisible">
				<div class="log-detail-data">
					<div class="team">
						<div v-for="player in log.radiantPlayers" v-if="player.damage">
							<div class="xp-chart">
								<span>{{player.xp_delta}}</span>
								<div class="inner" :style="`height: ${player.xpPercent}`"></div>
							</div>
							<div class="gold-chart">
								<span>{{player.gold_delta}}</span>
								<div class="inner" :class="player.gold_delta < 0 && 'negative'" :style="`height: ${player.goldPercent};`"></div>
							</div>
							<div class="damage-chart">
								<span>{{player.damage}}</span>
								<div class="inner" :style="`height: ${player.damagePercent}`"></div>
							</div>
					...
					<div class="team">
						<div v-for="player in log.direPlayers" v-if="player.damage">
							<div class="xp-chart">
								<span>{{player.xp_delta}}</span>
								<div class="inner" :style="`height: ${player.xpPercent}`"></div>
							</div>
					...
					<div class="zoom-box" :style="`height: ${imageHeight}`">
						<figure :style="`width: ${imageScale}00%`">
							...
							<template v-for="player in log.players">
								<figure class="log-detail-death-pos" v-for="position in player.positions" :style="`left: ${position.x}%; top: ${position.y}%; background-image: url(${player.heroIcon})`"></figure>
							</template>
						</figure>
					</div>
				</div>
